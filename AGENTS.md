...
本项目是基于 ESP32 的固定 4 路智能灌溉控制器应用项目，默认启用第 1、2 路。`old-docs/` 中的旧版设计文档只作为需求来源和背景材料；后续需求必须再次优化确认后再实现。新实现不继承旧代码、旧接口或旧数据格式的历史包袱。
...
- 旧文档中的 EC11 编码器方案不再作为当前实现依据。当前方案为固定 4 路板级配置、默认启用第 1/2 路，本地控制使用独立按键；第 3/4 路默认通过 Web/API/计划控制，除非后续明确增加独立本地按键硬件。

...
- 漏水监控：待机状态检测异常脉冲并告警。
- 计划浇水：每路最多 6 条计划，总计 24 个计划槽；今天/明天/后天可按单次跳过。
- Web/API：首页、近期计划、历史记录、计划配置、灌溉设置和状态 API；手动浇水合并在首页，业务页面使用 Esp32Base UI baseline，不复制基础库页面 CSS。
...
- 阀门 PWM 输出：第 1 路 GPIO13，第 2 路 GPIO14，第 3 路 GPIO16，第 4 路 GPIO27。当前代码假设输出有效即开阀。
- 流量计输入：第 1 路 GPIO34，第 2 路 GPIO35，第 3 路 GPIO36，第 4 路 GPIO39。这些输入脚没有内部上拉；YF-S201 等开漏输出传感器必须使用外部上拉。

...
- 漏水监控：待机状态检测异常脉冲并告警。
- 计划浇水：每路最多 6 条计划，总计 24 个计划槽；计划定义以单 blob 保存，计划执行结果按 `planId + 日期 + 分钟` 持久化，避免重启后重复执行或漏记已处理状态；重启/NTP 同步后仍处理宽限期内计划，`MISSED` 不持久化以允许时间回拨后重新评估。
- 单次跳过：API 已支持按 `planId + 日期` 跳过/取消跳过，日期必须是真实有效日期；跳过表按单槽更新并在启动时清理过期项。当前 Web 页面仍以计划配置为主，近期计划视图尚未作为页面能力承诺。
...
- 恢复出厂：GPIO0/BOOT 长按 3 秒直接执行“仅恢复配置”并重启；业务 Web/API 不提供恢复出厂入口，系统维护能力优先使用 `Esp32Base`。

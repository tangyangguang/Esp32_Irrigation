...
系统时间有效可以来自外部 RTC 或 NTP。上电后如果 RTC 时间有效，即使网络不可用，自动计划也可以运行；如果 RTC 和 NTP 都无效，自动计划不可用。
...
如果低流量或高流量策略是 `warn`，同一次 Zone 运行中同类 warning 事件最多写一次，避免持续异常时刷满事件日志。若策略是 `stop`，按故障停机流程写对应 stop 事件和浇水记录。

流量计异常：

```text
仅用于输入电气或计数异常
例如脉冲频率超过合理硬上限、输入抖动风暴、计数器状态异常
成立后全局锁定，必须用户手工清除
```

启动后没有脉冲或运行中无脉冲不是流量计异常，按 `no_water` 处理。
...
流量计校准和正常流量测定都属于维护运行。维护运行也必须有最大运行时间，避免页面断开或用户忘记停止导致持续放水。

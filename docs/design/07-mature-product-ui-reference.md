...
自动计划按时间停止
水量只用于观察、记录和异常判断
天气跳过只影响单次自动计划是否执行，不按天气自动调节水量或时长
```
...
结合成熟产品和当前产品诉求，本项目可以把天气能力收窄为“天气自动跳过计划”：
...
当前原型应从“天气参考”升级为“天气跳过”，但仍不能扩展到智能天气浇水或按天气调节水量。
...
因此，本项目采用“计划前天气跳过”，而不是 ET 智能浇水，也不把天气原因混入自动总控的人工暂停。
强风和低温虽然出现在部分成熟智能产品中，但不是最小可用天气跳过能力；本项目第一阶段不在 Web 原型中暴露，避免设置页复杂化。

#### 本项目建议策略

```text
1. 天气自动跳过是自动计划的 gate，不是计划生成器。
2. 手动浇水不受天气自动跳过影响。
3. 每个自动计划开始前，应使用最新可用天气数据做一次判断。
4. 如果设备无法获取新天气，允许使用未过期数据做计划判断；判断数据过期时不因为未知天气跳过计划。首页可继续显示最近一次天气，但必须标记更新时间或“仅供参考”。
5. 达到天气阈值时，跳过本次计划，并记录天气跳过原因。
6. 被天气跳过的计划不补跑，不移动到后面执行。
7. 每天固定时间刷新天气状态；下一次计划开始前仍会重新判断是否跳过。
8. 用户可以允许下一次计划执行，但不关闭后续天气自动跳过。
9. 用户可以关闭天气自动跳过；关闭后只保留天气展示，不自动跳过计划。
```
...
每天更新天气时间：08:00
计划前天气判断：计划开始前 60 分钟；若实现允许，也可在开始前再做一次轻量判断
降雨触发：降雨概率 >= 70% 且预计雨量 >= 5 mm
计划判断数据有效期：建议 6 小时；过期后不用于自动跳过计划
```
...
设置页：
  只保留启用开关、判断窗口、计划前判断时间、每日更新时间、计划判断数据有效期、降雨概率、预计雨量
```
...
```text
1. 首页显示天气跳过判断，明确只影响单次自动计划，不影响自动总控和手动浇水。
2. 首页显示当前运行概况和来源。

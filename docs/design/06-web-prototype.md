...
高级参数层：
  ...
  放在高级区域；原型评审阶段可默认展开，正式实现可按设备模式折叠。
```
...
8. POST /api/flow/calibration/sample，confirm=true。
9. 页面显示样本列表、偏差和合并建议。
10. 用户确认应用。
11. POST /api/flow/calibration/apply，confirm=true。
```
...
原型阶段所有分组默认展开，便于评审完整字段；后续正式实现可以根据设备模式决定是否折叠。所有数字输入必须在标签中体现单位，并在说明中写出支持范围。

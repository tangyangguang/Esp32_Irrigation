...
```text
lowThreshold = zone.normalFlowMlPerMin * settings.lowFlowPercent / 100
```

Running 稳定阶段中，如果 `flowMlPerMin` 持续低于 `lowThreshold` 超过 `settings.flowFaultConfirmSec`，低流量成立。
...
```text
highThreshold = zone.normalFlowMlPerMin * settings.highFlowPercent / 100
```

Running 稳定阶段中，如果 `flowMlPerMin` 持续高于 `highThreshold` 超过 `settings.flowFaultConfirmSec`，高流量成立。

...
LocalUi
  本地按键和屏幕的轻量交互。后置实现，只做选择、启动、停止和状态显示。
```

第一阶段交付目标是 Web/API 全功能闭环，包括配置、计划、校准、手动浇水、停止、记录、事件和故障清除。本地按键、屏幕显示和本地操作模式后置，不阻塞第一阶段 Web 功能实现。
...
当前设计没有高频掉电必须保存的数据。流量计脉冲、实时流速、当前运行秒数和队列状态都保存在 RAM；配置、浇水记录、事件日志和故障锁定只在明确业务边界写入。

因此当前软件不依赖外部 FRAM。控制板可以不加 FRAM。只有未来出现以下需求时，才重新评估：

```text
运行中每秒或每个脉冲都要掉电保存
掉电后必须恢复未完成浇水进度
掉电后必须保留本次未完成运行的精确累计水量
需要长期高频传感器采样日志
```

如果 PCB 为了实验预留 FRAM 焊盘，也应作为未使用扩展资源，不进入当前软件主路径。

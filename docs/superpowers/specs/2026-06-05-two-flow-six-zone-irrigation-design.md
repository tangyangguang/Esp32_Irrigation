...
`minValidFreqMilliHz` 用来处理低频边界：当喷头太少、流量计刚好偶发脉冲、或水流低于传感器可靠量程时，不应因为正 Offset 把极低频脉冲放大成有效出水量。默认值建议由实测决定；按用户当前透明流量计约 245 脉冲/L、标称下限约 1 L/min 估算，默认先设为 `4000`，即约 4 Hz。`maxValidFreqMilliHz = 0` 表示不启用上限检查；启用上限时只标记样本异常，不直接推断具体故障原因。
...
校准生成的候选参数也必须包含 `minValidFreqMilliHz` 和 `maxValidFreqMilliHz`。单点校准默认沿用当前 Flow 的有效频率范围；多点校准可把最低可信样本频率作为候选 `minValidFreqMilliHz`，但页面必须允许用户手工覆盖。

...
```text
当前参数     正在用于运行估算的参数集
候选参数     已生成或已输入、等待用户确认应用的参数集
上一套参数   最近一次被替换掉的当前参数，用于恢复
```

校准和候选参数编辑都只生成或替换候选参数，不直接改变当前参数。只有用户明确点击“设为当前”后，系统才把该路当前参数保存为上一套参数，再把候选参数写入当前参数。

候选参数和上一套参数按水路持久化保存，断电或重启后不丢失。默认状态下候选参数和上一套参数均为空；没有候选参数时页面显示“暂无候选参数”，没有上一套参数时隐藏或禁用恢复操作。

候选参数只保存最终参数值，不保存来源类型或复制来源水路。页面里的“从其他水路填入”只是候选编辑表单中的快捷填值动作：它把其他水路的当前参数填入表单，用户仍可继续修改，最后统一保存为候选参数。

候选参数等于当前参数时，应用操作必须没有副作用：不能覆盖上一套参数，不能写事件日志。页面应禁用或隐藏应用按钮，API 应返回明确错误。

恢复上一套参数时，当前参数和上一套参数互换。这样用户恢复后仍可再次切回，不引入多级历史。
...
范围：2-5
默认：5
说明：RAM 中最多保留的校准样本容量，只限制新增样本，不限制生成候选参数。
...
页面允许用当前所有有效样本生成候选参数；1 条有效样本也可以生成候选参数，但不能做误差交叉验证。
...
7. 点击生成候选参数。
8. 页面展示候选参数、每次样本误差、平均误差、最大误差和客观诊断建议。
9. 用户确认后应用候选参数到该水路当前参数。
```

交互确认设计：`停止并输入水量` 只是结束当前校准出水并进入样本录入，`保存样本` 只是保存本次 RAM 工作区样本；这两个动作不弹出二次确认。开始接水、清空样本、生成候选参数、应用候选参数、恢复上一套参数等动作继续保留二次确认。

校准样本是临时工作区，候选参数是持久化草稿。生成候选参数后，即使清空样本，也不清空候选参数。

同一组样本不得混用不同水路。工作区已有某一路样本时，开始采集其他水路应被拒绝并提示先清空样本；如果因为异常路径出现混合水路样本，生成候选参数必须失败。
...
- 候选参数和上一套参数持久保存，断电或重启后仍可查看和操作。
- 只有用户确认“设为当前”后，才改变该路当前运行参数。
- 应用候选参数和恢复上一套参数时，该水路必须处于空闲状态。
- 应用候选参数和恢复上一套参数必须记录业务事件，包含水路号、旧参数和新参数。

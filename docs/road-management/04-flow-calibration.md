...
校准、手工输入和复制操作都只生成或替换候选参数，不直接改变当前参数。只有用户明确点击“应用候选参数”后，系统才把该路当前参数保存为上一套参数，再把候选参数写入当前参数。
...
```text
manual       手工输入
calibration  校准样本计算生成
copied       从其他水路正在使用的当前参数复制
```

复制只允许从其他水路的当前参数复制到本路候选参数，不复制其他水路的候选参数，也不直接覆盖本路当前参数。`copied` 候选参数应记录来源水路号，便于页面和日志说明来源。
...
- 只有用户确认“应用候选参数”后，才改变该路当前运行参数。
- 应用候选参数和恢复上一套参数时，该水路必须处于空闲状态。
- 应用候选参数和恢复上一套参数必须记录业务事件，包含水路号、旧参数和新参数；应用候选参数还应记录候选来源。

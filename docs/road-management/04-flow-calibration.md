...
达到目标有效样本数后，页面允许计算推荐参数。样本数不足时也可预览，但置信度降低。
...
当只有 1 个有效样本时，系统无法真正区分启动阶段和稳定阶段，只能给出低置信度结果：使用整次样本反推整体每升脉冲，并按该比例给出启动阶段水量。页面必须提示建议至少采集 3 次。
...
6. 重复到目标样本数。
7. 点击计算推荐参数。
8. 页面展示推荐参数、每次样本误差、平均误差、最大误差和置信度。
9. 用户确认后应用到该水路配置。
```

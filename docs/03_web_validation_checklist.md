# Web 功能验证清单

本清单用于烧录固件后验证 `/irrigation` 业务首页和 `/esp32base` 系统工具入口。验证前先确认设备已进入 Web ready 状态。

## 首页与导航

- 打开 `/`：应进入灌溉业务首页。
- 打开 `/irrigation`：应显示中文业务首页，包含网络信息、阀门状态、手动浇水和当前告警提示。
- 业务导航不应显示单独的手动浇水页；手动浇水应在首页完成。
- 打开 `/irrigation/plans`：应以单个表格显示今天、明天、后天的近期计划状态和单次跳过操作。
- 顶部业务导航顺序应为：首页、近期计划、历史记录、计划配置、灌溉设置。
- 打开 `/irrigation/data`：应只显示最近浇水记录，导航标题为“历史记录”。
- 打开 `/irrigation/plan-config`：应显示 8 个计划配置入口。
- 打开 `/irrigation/settings`：应显示灌溉设置页面，按水路、流量计量和检测参数聚合配置，不显示维护或恢复出厂表单。
- 打开 `/esp32base`：应显示基础库融合/系统工具入口。
- 业务页应使用 `Esp32BaseWeb::setHeadExtraCallback()` 注入业务 CSS，底部应保留 `Esp32Base` 默认页脚导航：`Status`、`Logs`、`App Config`、`System`，不应被业务 CSS 或中文标签覆盖。

## 状态区

- 能看到 WiFi 名称、RSSI、IP 地址、当前时间、阀门状态和记录/事件容量。
- `GET /api/v1/status` 返回合法 JSON。
- 状态 JSON 中包含 `wifi.ssid`、`wifi.rssi`、`wifi.ip`、`time.current`、每路 `flow_l_min`、`records.count` 和 `events.count`。
- 禁用 `ESP32BASE_ENABLE_WIFI` 构建时，状态 JSON 仍必须是合法 JSON，`wifi.connected=false` 且 `ssid/ip` 为空字符串。

## 手动控制

- 默认第 1 路和第 2 路都启用，首页手动浇水应默认按当前启用水路参与。
- 关闭其中一路后，停用水路仍应在两路结构中可见，但不能被启动；启用水路仍可单独浇水。
- 两路启用时，应能只浇第 1 路、只浇第 2 路、两路同时浇水。
- 停止 R1、停止 R2、Stop All 表单提交后返回 `/irrigation`。
- 操作后 `/api/v1/events` 应出现 `water_start` 或 `water_stop`。
- 操作后 `/api/v1/records` 应出现浇水会话记录。

## 配置

- 修改每路启用状态、默认模式、每路名称、每路默认时长、水流异常窗口、漏水窗口、漏水脉冲阈值后保存。
- 设置页表单应提交到 `/irrigation/settings/config` 并返回设置页；`POST /api/v1/config` 应始终返回 JSON 成功或错误，不做页面跳转。
- 刷新页面后配置值应保持。
- 重启后配置值应保持。
- 无效范围必须拒绝，例如 `flow_no_pulse_timeout_s=0` 或 `61`。
- 基础库系统导航应提供认证设置入口；业务项目不保存基础库鉴权密码。
- 通过 `/esp32base/auth` 修改账号密码后，新账号密码立即生效，旧凭据后续请求应失效。

## 浇花计划

- 8 个计划都应显示。
- 每个计划可单独保存启用状态、时间、R1/R2 时长、模式、循环天数和周期内执行日；默认计划内容应包含两路。
- 近期计划应按当前启用水路显示有效执行内容；停用水路不应导致整个计划失败。
- 循环开始日期应使用日期选择框；出厂默认且未启用、未执行过的计划在设备已同步时间时应默认显示今天。
- 近期计划表格中，每个今天、明天、后天的未执行计划都应能单独跳过；已跳过计划应能取消跳过。
- 保存或跳过操作后 `/api/v1/events` 应出现 `plan_changed`。
- `GET /api/v1/plans` 返回 8 个计划的原始配置字段。

## 记录与查询

- 页面应显示最近浇水记录。
- 页面不应显示 JSON 或 CSV 导出按钮。
- 首次启动时记录文件应能自动初始化，启动过程不应触发 task watchdog。
- `GET /api/v1/records?limit=10` 返回合法 JSON。
- 页面不应显示系统事件；设备级诊断优先使用 `Esp32Base` 日志。
- `GET /api/v1/events?limit=20` 作为内部事实流诊断 API 保留，并返回合法 JSON。
- 顶部业务导航不应显示调试页；`GET /api/v1/status` 作为隐藏状态 API 保留。

## 异常与维护

- 清除已处理异常表单提交后返回当前业务页面，并写入 `alert_clear` 事件。
- 业务页面不提供维护或恢复出厂入口。
- GPIO0/BOOT 触发恢复出厂 pending 后，配置保存、手动浇水、停止、清除告警、计划保存和计划跳过接口都应返回 `factory_reset_pending`，不再接受新的持久化写入。

## 未实机验证项

- 阀门实际电平是否与 `HIGH=开阀` 一致。
- YF-S201 脉冲计数是否稳定。
- 默认 10 秒无脉冲关阀是否符合真实水流启动延迟。
- GPIO0 长按 3 秒应直接执行仅配置恢复并重启；实机验证是否容易误触，以及是否保留记录和事件。
- 按住 GPIO0 上电或重启后，固件运行 3 秒不应触发恢复出厂；松开后再次长按 3 秒才允许触发。

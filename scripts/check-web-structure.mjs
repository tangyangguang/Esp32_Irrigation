...
assert(planHeader.includes('bool exists'), 'plan items should distinguish uncreated slots from disabled plans');
assert(planHeader.includes('bool create(uint8_t road') && planHeader.includes('bool remove(uint8_t index)'), 'plans should support create and delete operations');
...
assert(plans.includes('plan.exists = false'), 'default plan slots should be uncreated');
assert(scheduler.includes('!plan.exists || !plan.enabled'), 'scheduler should ignore uncreated plan slots');
assert(web.includes("action' value='create'"), 'plan configuration should expose explicit plan creation');
assert(web.includes("action' value='delete'"), 'plan configuration should expose explicit plan deletion');
assert(web.includes("confirm('确认删除该计划？')"), 'plan deletion should require browser confirmation');
assert(web.includes('writePlanRoadGroup'), 'plan configuration should be grouped by road');
assert(web.includes('暂无计划'), 'plan configuration should render empty road groups');
assert(web.includes('计划配置页按水路分组'), 'plan edit page should describe road-grouped configuration');
assert(web.includes('.roadcfggrid') && web.includes('.roadcfg.disabled'), 'settings page should use compact road cards with disabled styling');
assert(web.includes('Esp32BaseWeb::setHeadExtraCallback(writeIrrigationHeadExtra)'), 'irrigation pages should inject scoped lightweight business CSS');

...
Stop All 确认后立即关闭所有 Zone，并中止校准/学习。
```

本地启动和停止都必须有二次确认，但确认流程不能做成复杂菜单。第一版统一采用同键二次确认：

```text
启动当前 Zone：
  第一次按 Button 3：进入 confirm_start，屏幕显示目标 Zone、Flow 和默认时长。
  5 秒内再次按 Button 3：重新做服务端同等校验，通过后启动。
  按 Button 1/2 切换 Zone、按 Button 5 切换信息页或确认超时：取消确认。

停止当前 Zone：
  当前选中 Zone 正在运行时，第一次按 Button 3：进入 confirm_stop_zone。
  5 秒内再次按 Button 3：停止该 Zone。
  切换 Zone 或确认超时：取消确认。

Stop All：
  第一次按 Button 4：进入 confirm_stop_all，屏幕显示将关闭所有运行 Zone 并中止校准/学习。
  5 秒内再次按 Button 4：执行 Stop All。
  其他选择键或确认超时：取消确认。
```

确认状态必须绑定 `zoneId` 和动作类型，不能因为确认期间选中项变化而误操作其他 Zone。执行前仍要重新读取当前状态；例如确认启动时 Flow 变忙，则拒绝启动并显示 `flow_busy`。

# 专题 01：小程序完整配置管理（水路 / 计划 / 系统参数）

状态：实施中（三仓源码与定向检查完成；实机 MQTT 验收、服务端部署、手机实测待授权）
...
**未覆盖边界（集成/验收阶段统一收口）**：服务端未部署新契约产物；小程序未做隔离页面走查与手机实测；固件未实机验证命令落盘、revision 冲突、参数待应用和记录建议值展示；禁用水路真实名称当前依赖 config 快照（state.zones 只投影启用路），禁用路改名回退显示“水路 N”，是否需要扩展投影待后续评估。

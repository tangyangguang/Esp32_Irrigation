...
// Business records are stored in the SDK linear (IR/v1) slot: 24-byte stream
// header followed by business bytes. The first four business bytes carry the
// run duration in seconds (little endian); the remainder is the fixed codec
// payload. One durable store per stream serves both local history and
// platform replay; no second outbox is maintained.
inline void putDuration(uint8_t* bytes, uint32_t duration) {
    for (unsigned n = 0; n < 4; ++n)
        bytes[n] = uint8_t(duration >> (n * 8));
}
inline bool decode(const uint8_t* bytes, size_t length, size_t expected,
    ...
    if (!iot_device::linearRecordEncoding().decode(bytes, length, fact) ||
        fact.dataBytes != expected || fact.dataBytes < 4 || !fact.sequence ||
    ...
    for (unsigned n = 0; n < 4; ++n)
        timing.durationSec |= uint32_t(fact.data[n]) << (n * 8);
    return true;
}
}  // namespace irrigation_fact

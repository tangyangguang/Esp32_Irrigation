...
void IrrigationWeb::flowCalibration() {
    ...
    Esp32BaseWeb::sendChunk(
        ...
        ".cal-start-form{display:grid;grid-template-columns:minmax(520px,1.25fr) minmax(280px,.75fr);gap:16px;align-items:stretch}.cal-start-fields{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.cal-start-fields .field{margin:0}.cal-start-fields label{font-weight:500}.cal-start-fields select,.cal-start-fields input{width:100%;margin-top:6px}.cal-start-picker>.actions{margin-top:12px}.cal-start-info{display:flex;flex-direction:column;justify-content:center;padding-left:16px;border-left:1px solid var(--eb-line-soft);color:var(--eb-muted);font-size:12px;line-height:1.55}.cal-start-info b{color:var(--eb-text);font-size:13px;font-weight:400}"
        ...
        ".cal-live .metrics{grid-template-columns:repeat(2,minmax(0,1fr))}.cal-pending{grid-template-columns:1fr}.cal-sample-toolbar{align-items:flex-start}.cal-sample{grid-template-columns:1fr auto}.cal-sample-actions{grid-column:2;grid-row:1/span 5}.cal-parameter-grid{grid-template-columns:1fr}"
        "}"
        "@media(max-width:560px){.cal-start-fields{grid-template-columns:1fr}.cal-current-toolbar{align-items:stretch;flex-direction:column}.cal-current-toolbar button{width:100%}.cal-current{grid-template-columns:1fr}.cal-current-fact{border-right:0;border-bottom:1px solid var(--eb-line-soft)!important}.cal-current-fact:last-child{border-bottom:0!important}.cal-result-head{align-items:flex-start}.cal-result-grid{grid-template-columns:1fr}.cal-result-fact{border-right:0;border-bottom:1px solid var(--eb-line-soft)!important}.cal-result-fact:last-child{border-bottom:0!important}.cal-sample{grid-template-columns:1fr}.cal-sample-actions{grid-column:1;grid-row:auto;justify-content:flex-start}}"
        "</style>");
    const IrrigationConfig* config = g_app->configuration();
    ...
    if (status.active && !active) {
        ...
        if (hasEligibleZone) {
            ...
            Esp32BaseWeb::sendChunk("</select></p><p class='field'><label>自动停止方式</label><select id='cal-stop-mode' name='stop_mode'><option value='time'>按最长时间</option><option value='volume'>按目标容量</option></select></p><p id='cal-time-field' class='field'><label>最长出水时间（分钟）</label><input type='number' name='duration_minutes' min='1' max='10' step='1' value='10' required><small>到时自动停止，运行中也可随时停止。</small></p><p id='cal-volume-field' class='field' hidden><label>目标容量（L）</label><input type='number' name='target_liters' min='1.000' max='1000.000' step='0.001' value='5.000' inputmode='decimal' disabled><small>按当前校准系数估算，达到目标后自动停止；仍保留 10 分钟硬上限。</small></p></div><div class='actions'><input type='submit' value='开始新样本'></div></div><div class='cal-start-info'><b>已保存 "); sendUnsigned(calibration.sampleCount()); Esp32BaseWeb::sendChunk(" / 10 个样本，还可增加 "); sendUnsigned(FlowCalibrationService::kMaximumSamples - calibration.sampleCount()); Esp32BaseWeb::sendChunk(" 个</b><span>每个样本可重新选择水路。稳态判定：每个窗口 "); sendUnsigned(config->calibrationStability.windowSec); Esp32BaseWeb::sendChunk(" 秒，需连续 "); sendUnsigned(config->calibrationStability.requiredWindows); Esp32BaseWeb::sendChunk(" 个，窗口速率波动不超过 "); sendUnsigned(config->calibrationStability.allowedVariationPercent); Esp32BaseWeb::sendChunk("%。目标容量只是自动停止依据，实际接水量仍以量具读数为准。</span></div></form><script>(function(){var mode=document.getElementById('cal-stop-mode'),time=document.getElementById('cal-time-field'),volume=document.getElementById('cal-volume-field');if(!mode||!time||!volume)return;function sync(){var byVolume=mode.value==='volume',timeInput=time.querySelector('input'),volumeInput=volume.querySelector('input');time.hidden=byVolume;volume.hidden=!byVolume;timeInput.disabled=byVolume;timeInput.required=!byVolume;volumeInput.disabled=!byVolume;volumeInput.required=byVolume}mode.addEventListener('change',sync);sync()})();</script>");
        } else {

...
| 进行中 | C：SDK 平台职责接入 | 设备阶段 `2133e72` 已提交推送，SDK 命令/ACK 信封、UUID、UTC 解析与受控型号生成已通过目标构建、33 个共享向量及 14 项相关 Native 检查；SDK 双流会话 a076543 已提交推送；设备会话、状态/证据发布和双流接入阶段 5180992 已提交推送并验证，用户已确认维持字节预算、允许历史条数变化。接下来替换设备逐命令 NVS journal；保留重连幂等、启动截止点和先关输出后终态。 |
...
诊断状态直接读取 Base 固件信息、启动/复位、累计运行时长、当前/最低堆、WiFi 连接尝试、MQTT 计数和错误、NTP 与 WDT。随现有状态队列在连接后及 24 秒周期发布，无新任务或网络生命周期。目标链接 Flash 1615659 B、RAM 100516 B，仍因容量失败，未做硬件动作。

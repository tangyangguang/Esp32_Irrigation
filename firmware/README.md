...
当前架构重构目标 Core 3 TLS 镜像 1,567,920 B，静态 RAM 95,196 B，OTA 单槽余 201,552 B（11.39%）。独立控制任务另使用 4096 B 运行栈及任务控制块，资源取舍优先保证控制时序。配置/控制器 46 项、执行任务宿主检查、真实 Base/SDK 存储故障及恢复检查通过；最终镜像已完成授权 Web OTA 及一次合并的无水短时检查。真实水路精度、物理波形和长期稳定性未验证。
...
当前架构重构按用户确认的评审实施并已推送：设备 8238ee2（执行任务与参数职责）、a71d376（双 Store 独立故障/恢复及正式文档），Base d2555ea（参数持久化与生效状态分离）。最终镜像已 OTA 到授权完整硬件实验板，运行 app1 / valid；必要宿主检查、目标编译与合并无水短时检查均通过，详见本页末节。

当前无进行中或待办的本轮开发项：设备、基础库及平台/小程序配套已完成收尾并通过必要检查。真实水路与长期试运行由用户按需开展，不作为本轮交付前置条件；独立专业校准工具仍属后续任务。
...
| 状态 | 交付结果 / 责任 | 尚缺工作与边界 |
| --- | --- | --- |
| 用户试运行 | 现场边界 | 本轮使用完整硬件但未接水路的授权实验板，OTA 与短时无水检查通过。早期核心板证据仅作历史记录；长稳、满容量、反复断电和真实水路由用户按需验证。 |
...
已删除定义、协议校验及模拟器的独立 `learning` 活动和 `maintenance_activity` 拒绝分支；小程序首页/详情不再显示已删除的活动，维护页移除过期参数数量与校准流程措辞。普通浇水建立基准的业务保留，`state.calibration` 仍只表示每升脉冲数，没有新增专业校准接口。

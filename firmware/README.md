...
| 状态 | 交付结果 / 责任 | 尚缺工作与边界 |
| --- | --- | --- |
| 进行中 | B：Base + 设备完整功能资源优化与工具链准入 | Base 安全资源优化已提交 `5469569`；新受控 Core 3 包已固定，设备候选完成链接但 Flash 1581831 B 仍超 OTA 槽 8967 B，按 ELF 达到 8% 余量还差约 132 KiB。用户再次明确完整功能不变，继续审查两项目的数据布局、重复实现、缓冲生命周期和链接成本；未改变分区或记录容量，候选仍未提交为可试运行版本。 |
...
当前 Core 3 候选 RAM 92852 B、Flash 1579175 B：相对第二轮起点，RAM 减少 12728 B、Flash 减少 2656 B；其中页面处理增加 Flash 6224 B，JSON 收敛等减少 8880 B。仍超槽 6311 B，8% OTA 余量仍未满足，不能视作可试运行固件。保留完整功能、原分区及 Store/FileLog 预算，继续 SDK 去重和资源审查；设备 Core 3、维护安全接线候选尚未作为成品提交。

...
| 状态 | 交付结果 / 责任 | 尚缺工作与边界 |
| --- | --- | --- |
| 进行中 | C：SDK 平台职责接入 | 设备阶段 `2133e72` 已提交推送，SDK 命令/ACK 信封、UUID、UTC 解析与受控型号生成已通过目标构建、33 个共享向量及 14 项相关 Native 检查；继续会话、命令生命周期、状态/证据发布，替换设备逐命令 NVS journal；保留重连幂等、启动截止点和先关输出后终态。 |
...
当前目标为 classic ESP32、**8 MiB 物理 Flash**，两个 3 MiB OTA 槽、1920 KiB LittleFS，保留全部业务功能、Store/FileLog 容量与安全边界，不继续压缩资源。Core 3.3.8 固定 TLS 工具链为 `esp32-core-3.3.8-tls-a33c13b24f5db543`，保留证书日期校验及出站 TLS，详见 [Base 工具链文档](../../../foundation/Esp32Base/docs/14_tls_toolchain.md)。原 N4 BOM 和本机现接 4 MiB 样机不满足新目标；未改写硬件事实，也未烧录不匹配镜像。

...
当前架构重构目标 Core 3 TLS 镜像 1,567,936 B，静态 RAM 95,196 B，OTA 单槽余 201,536 B（11.39%）。独立控制任务另使用 4096 B 运行栈及任务控制块，资源取舍优先保证控制时序。配置/控制器 46 项、执行任务宿主检查、真实 Base/SDK 存储故障及恢复检查通过；最终镜像尚待本轮 OTA。此前物理证据按各节标注，不套用于新执行架构。
...
当前架构重构按用户确认的评审实施。第一阶段（执行任务、参数职责和生效反馈）已提交：设备 8238ee2，Base d2555ea。双 Store 独立故障/恢复及正式文档已完成，当前准备最终 OTA 与短时验证。此前 4dd2194/a027f68 的完成结论仅属于上一轮精简，不代表本次新架构已实机通过。

- 当前进行中：最终 OTA 与短时检查；代码、宿主检查及目标编译已完成。

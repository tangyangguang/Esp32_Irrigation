...
| 状态 | 交付结果 / 责任 | 尚缺工作与边界 |
| --- | --- | --- |
| 待决策 | B：设备工具链与资源准入 | Base 安全资源优化已提交 `5469569`；新受控 Core 3 包已固定，设备候选完成链接但 Flash 1581831 B 仍超 OTA 槽 8967 B，按 ELF 达到 8% 余量还差约 132 KiB。按用户要求先完成 Base 优化再讨论剩余缺口；未改变分区或记录容量，候选仍未提交为可试运行版本。 |
...
灌溉最终候选 RAM 105580 B、Flash 1581831 B，比此前候选分别下降 808 B、30408 B；仍不满足 OTA 槽及 8% 余量门禁（ELF 口径尚缺约 132 KiB，最终 bin 可能更大）。当前候选已经引用固定产物 `esp32-core-3.3.8-tls-a33c13b24f5db543`；完整哈希及使用边界由 [Base 工具链文档](../../../foundation/Esp32Base/docs/14_tls_toolchain.md)维护。当前保留原分区、业务 Store、FileLog 预算和不少于 128 KiB 的 FS 安全余量；此前未采用的容量草案不是当前方案。

下一步需讨论剩余资源缺口的处理路线；SDK 协议去重、双流同步、定义/服务端/小程序适配仍未完成，不能把本轮 Base 优化当作完整灌溉交付。设备侧 Core 3/维护接线等未完成改动保留在工作区，未强行提交或烧录。

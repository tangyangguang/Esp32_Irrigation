...
当前源码计量简化已完成：目标 Core 3 TLS 镜像 1,569,440 B，RAM 98,396 B，OTA 单槽余 200,032 B（11.30%）。控制器/配置/协议 54 项、实际 Base 存储宿主检查及资源还原检查通过；类型定义/模拟器 38 项、小程序 10 项及类型检查通过。未烧录、未部署；下文旧实机证据不代表本轮实现已上板。
...
| 本轮状态 | 可验收阶段 | 边界 |
|---|---|---|
| 待办 | 本地页面收敛、记录故障准入和 MQTT 状态/补发调度 | 保留离线业务、安全保护和当前平台契约 |
| 待办 | Esp32Base 责任层优化、主目标资源比较和交付 | 按实际链接占用定位；定向验证，不运行长时实验 |

本轮配置 JSON 仅保存 schema_version、revision、zones、plans；系统参数只保存于 NVS。格式为 schema 5，不读取或迁移旧配置；无当前配置时初始化，已有无效配置时保持安全未就绪并明确报错，不自动格式化。更新旧样机需另行确认目标配置初始化，不能用本轮构建冒充已升级。
...
- 必要审计事实：`irrigation-audit` Store v2，固定 52 B payload、48 KiB 逻辑预算；保存自动计划运行/跳过、自动总控、计划修改和水路基准保存；

...
当前架构重构目标 Core 3 TLS 镜像 1,567,936 B，静态 RAM 95,196 B，OTA 单槽余 201,536 B（11.39%）。独立控制任务另使用 4096 B 运行栈及任务控制块，资源取舍优先保证控制时序。配置/控制器 46 项、执行任务宿主检查、真实 Base/SDK 存储故障及恢复检查通过；最终镜像已完成授权 Web OTA 及一次合并的无水短时检查。真实水路精度、物理波形和长期稳定性未验证。
...
当前无进行中或待办的本轮开发项；真实水路与长期试运行由用户按需开展，不作为本轮交付前置条件。独立专业校准工具仍属后续任务。
